...
# ----------------------------------------------------------------
ggraph <-
function( data=NULL, vertices=NULL, edges=NULL, edge.list=NULL, label=character(0), type="regular",
            algorithm="brute" ) {
	
	val <- NULL
	
	type.choices <- c("regular","composite")
	if( !is.element(type, type.choices ) ) stop ( "Invalid graph type" )
	...
	if( type == "composite" ) {
		...
		if( !is.null(data) ) {
			...
			vertices <- as.nodes( data )
			cat("Computing the Gabriel Graph...\n")
			edges <- switch(algorithm,
                heuristic = gabriel.2( data[, -ncol(data)] ),
                gabriel.1( data[, -ncol(data)] ))
		}
		else {
		...
		for( s in node.list ) {
			...
			#sc <- vector()
			#for( r in a.list[[s]] )
			#	sc <- append( sc, get.nodes(vertices, r)@class )		
...
#-----------------------------------------------------------------------------------------
	...
	if (!add ) {
		if( normalized )
			...
			plot( data[,2], data[,3], axes=FALSE, xlab="", ylab="" )
	...
	# Add class labels
	class.list <- unique( sapply(x@vertices, FUN=function(x) x@nclass) )
	class.b.ids <- sapply( get.nodes(x@vertices, nclass=class.list[2]), FUN = function(x) x@name )
	data.b <- data[ is.element( data[,1], class.b.ids ), ]
	if( is.vector(data.b) )
		points( data.b[2], data.b[3], pch = 16 )
	else
		points( data.b[,2], data.b[,3], pch = 16 )
	...
	if ( ids ) {
		...
		if( !mis.labels )
			text( data[,2:3] + xy.offset(data[,2],data[,3],dr), labels=node.labels, cex=cex )
		else
			text( data[,2:3] + xy.offset(data[,2],data[,3],dr), cex=cex )
	}
...
dumpMethod("composite.ggraph", "ggraph")
...
######
			MDS for high-dimensional data
			Hyperplanes for decision boundary
######
...
my.nodes <- NULL
my.nodes <- vector("list")
for(i in 1:5) {
	my.nodes[[i]] <- node(i, coord=gm1[i,1:2], nclass=gm1[i,3])
}
my.nodes

my.edges <- matrix(c(1,2,1,5,2,3,2,4,4,5),ncol=2, byrow=T)

# select nodes of given name
my.nodes[ sapply(my.nodes, FUN = function(x) x@name == 4)]
length( my.nodes[ sapply(my.nodes, FUN = function(x) x@nclass == 1)] )


# --------------------------------------------------

my.nodes.2 <- list( node(id=1, nclass=0), node(id=2, nclass=1), node(id=3, nclass=0), node(id=4, nclass=1),
						  node(id=5, nclass=1), node(id=6, nclass=1), node(id=7, nclass=0), node(id=8, nclass=0),
						  node(id=9, nclass=0))

my.edges.2 <- matrix(c(1,2,1,7,1,9,2,3,2,7,3,4,3,6,4,5,5,6,6,7,7,8,8,9), ncol=2, byrow=T)

ug.2 <- ggraph( vertices=my.nodes.2, edges=my.edges.2, label="Toy-graph" )
ug.2@adj.list[[3]]
ug.2@vertices

s <- ug.2@vertices[ sapply(ug.2@vertices, FUN = function(x,y) x@name == y, y=3) ]@nclass
sapply(s, FUN=function(x) x@nclass)

sapply(ug.2@adj.list[[1]], FUN=function(x) ug.2[ sapply(ug.2@vertices, FUN = function(x) x@name == 4)]@class)

# -------------------------------------------

my.nodes.3 <- as.nodes( m1 )
ug.3 <- ggraph( my.nodes.3, m1.gg, "Ripley-full" )
plot.graph(ug.3)
gabriel.plot( m1[,-3], m1.gg, m1[,3] )
plot(ug.3, ids=T, normalized=F, title=T)

ug.2 <- ggraph( vertices=my.nodes.2, edges=my.edges.2, label="test-graph-2" )
ug.m1 <- ggraph( data = m1[1:10,], label="test-graph-3" )
ug.m1
plot(ug.m1, ids=T, title=T)

ug.3 <- ggraph( data = m1, label="Ripley-full" )
ug.3
plot.graph(ug.3)

# ----------------------------------------------------------------

subgraph(ug.3, class="class-0")
subgraph(ug.3, topology="normal")
s1.ug.3 <- subgraph(ug.3, class="class-0", topology="border")
subgraph(ug.3, edit.type="same.class")

plot(ug.3, title=T)
plot(subgraph(ug.3, edit.type="boundary"), title=T)
plot(subgraph(ug.3, edit.type="same.class"), title=T)
plot(subgraph(ug.3, edit.type="same.class.topology"), title=T)
plot(subgraph(ug.3, class="class-0"), title=T)
plot(subgraph(ug.3, class="class-0", topology="normal"), title=T)
plot(subgraph(ug.3, class="class-1"), title=T)
plot(subgraph(ug.3, class="class-1", topology="normal"), title=T)
plot(subgraph(ug.3, class="class-0", topology="border"), title=T)
plot(subgraph(ug.3, class="class-1", topology="border"), title=T)

plot(ug.2, title=T, ids=T)
plot(subgraph(ug.2, edit.type="boundary"), title=T, ids=T)
plot(subgraph(ug.2, edit.type="same.class"), title=T, ids=T)
plot(subgraph(ug.2, edit.type="same.class.topology"), title=T, ids=T)
plot(subgraph(ug.2, class="class-0"), title=T)
plot(subgraph(ug.2, class="class-0", topology="normal"), title=T)
plot(subgraph(ug.2, class="class-1"), title=T)
plot(subgraph(ug.2, class="class-1", topology="normal"), title=T)
plot(subgraph(ug.2, class="class-0", topology="border"), title=T)
plot(subgraph(ug.2, class="class-1", topology="border"), title=T)
...
plot(subgraph(ug.2, edit.type="same.class"), title=T, ids=T)
s <- find.components(subgraph(ug.2, edit.type="same.class"))

s <- find.components(subgraph(ug.3, edit.type="same.class.topology"))

ss <- sapply( get.nodes(ug.2@vertices, s[[4]]), FUN=function(x) x@coord )
apply( sapply( get.nodes(ug.3@vertices, s[[10]]), FUN=function(x) x@coord ), 1, mean )

# -------------------------------------------------

sug.2 <- composite.ggraph( ug.2, "class")
sug.2t <- composite.ggraph( ug.2, "class.topology")

cug.3 <- composite.ggraph( ug.3, "class")
cug.3t <- composite.ggraph( ug.3, "class.topology")
plot( cug.3, ids=F, title=T, cex=1, dr=.05)

plot( subgraph( ug.3, edit.type = "same.class" ), title=T )
plot( composite.ggraph( ug.3, "class"), ids=T, title=T )
